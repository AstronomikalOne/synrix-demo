...
print()
print("  Three layers reached the same conclusion independently:")
print()
print(f"    RETRIEVAL   Best cosine similarity across {N_CORPUS:,} bearing records: {best_sim:.4f}.")
print("                Foreign reading was out-of-distribution before routing ran.")
print()
print("    ROUTING     Rule engine assigned a different processing class than every")
print("                historical bearing record in the corpus. No rule was written")
print("                for this input type.")
print()
print(f"    GATE        {_gate_kb} KB student, trained only on bearing data, disagreed with")
print("                the teacher's routing decision. The outcome was outside its")
print("                learned distribution.")
print()
print("  No coordination between layers. No rule written for this input type.")
print()
print(f"  Lattice write p50: {float(np.percentile(write_times, 50)):.0f} µs  |  "
      f"AION insert: {N_CORPUS/idx_s:,.0f} vec/s  |  "
      f"Rule engine warm p50: {p50_warm:.1f} µs ({rules_pps:,} pps)")
print()
print("  Operational verdict: foreign domain. In a deployed system — halt and preserve state.")
print()
